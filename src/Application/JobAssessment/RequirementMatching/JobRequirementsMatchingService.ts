...
export class JobRequirementsMatchingService implements IJobRequirementsMatchingService {
    ...
    public async matchSingle(
        ...
        if (result.status === PipelineStepStatus.Failed) {
            ...
            const reason = result.reason ?? `Requirement matching failed for ${requirement.formattedName()}`;
        ...
        if (!context.match) {
            const reason = `Requirement matching produced no match: ${requirement.formattedName()}`;
    ...
    private logMatch(match: IJobRequirementMatch): void {
        this.logger.info(`\t- ${match.requirement.formattedName()}`);
