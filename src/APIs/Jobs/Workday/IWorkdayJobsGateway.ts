...
//     getJobPostings(request: IJobsRequest): Promise<WorkdayJobsResponse>;
//     getJobDetails(url: string): Promise<WorkdayJobDetail>;
// }
...
export class WorkdayJobsGateway implements IJobSource {
    ...
    private readonly jobsUrl = "https://workday.wd5.myworkdayjobs.com/wday/cxs/workday/Workday/jobs";

    async getJobPostings(request: IJobsRequest): Promise<WorkdayJobsResponse> {
        ...
        if (response.status < 200 || response.status >= 300) {
            throw new Error(`Workday jobs API returned HTTP ${response.status} ${response.statusText}`);
        }
    ...
    async getJobDetails(url: string): Promise<WorkdayJobDetail> {
        ...
        if (!response.ok) {
            throw new Error(`Workday job page returned HTTP ${response.status} ${response.statusText}`);
        }
